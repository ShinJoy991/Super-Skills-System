...
public class SectMerchantContainer implements Container {
    ...
    private final SectVillagerMenu menu;

    public SectMerchantContainer(Merchant merchant, SectVillagerMenu menu) {
...
//        System.out.println("setItem1.1 "+this.merchant.getTradingPlayer().level().isClientSide + " " + p_40013_);
...
//            System.out.println("setItem2 " + this.merchant.getTradingPlayer().level().isClientSide);
        }
//        System.out.println("setItem3 " + this.merchant.getTradingPlayer().level().isClientSide);
        if (this.isPaymentSlot(p_40013_)) {
            this.updateSellItem();
//            System.out.println("setItem4 " + this.merchant.getTradingPlayer().level().isClientSide);
        }
...
//        System.out.println("UpdateSellItem " + this.merchant.getTradingPlayer().level().isClientSide);
        if (this.itemStacks.get(0).isEmpty()) {
            itemstack = this.itemStacks.get(1);
            itemstack1 = ItemStack.EMPTY;
//            System.out.println("itemstack is empty "+ this.merchant.getTradingPlayer().level().isClientSide + " " + itemstack.getItem());
        }
        else {
            itemstack = this.itemStacks.get(0);
            itemstack1 = this.itemStacks.get(1);
//            System.out.println("itemstack is not empty"+ this.merchant.getTradingPlayer().level().isClientSide);
        }

        if (itemstack.isEmpty()) {
            this.setItem(2, ItemStack.EMPTY);
            this.futureXp = 0;
//            System.out.println("setItem 2 to EMPTY, futureXp set to 0"+ this.merchant.getTradingPlayer().level().isClientSide);
        }
        else {
            MerchantOffers merchantoffers = this.menu.getOffers();
            if (!merchantoffers.isEmpty()) {
//                System.out.println("Merchant level update is client? " + this.merchant.getTradingPlayer().level().isClientSide);
...
//                    System.out.println("MerchantOffer from reversed getRecipeFor: " + merchantoffer);
